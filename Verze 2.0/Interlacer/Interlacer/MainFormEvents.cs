...
namespace Interlacer
{
    ...
    public partial class MainForm : Form
    {
        ...
        private void pictureListViewEx_ItemSelectionChanged(object sender, ListViewItemSelectionChangedEventArgs e)
        {
            try
            {
                Point mousePos = pictureListViewEx.PointToClient(Control.MousePosition);
                ListViewHitTestInfo hitTest = pictureListViewEx.HitTest(mousePos);
                int columnIndex = hitTest.Item.SubItems.IndexOf(hitTest.SubItem);

                if (columnIndex == 0 && (hitTest.Item.ImageKey.Equals("expandPlus.png") || hitTest.Item.ImageKey.Equals("expandMinus.png")))
                    pictureListViewEx.Items[hitTest.Item.Index].Selected = false;
            }
            catch { }
            finally
            {
        ...
        private void pictureListViewEx_MouseUp(object sender, MouseEventArgs e)
        {
            try
            {
                Point mousePos = pictureListViewEx.PointToClient(Control.MousePosition);
                ...
                if (columnIndex == 0)
                {
                    if (hitTest.Item.ImageKey.Equals("expandPlus.png"))
                    {
                        expandItem(hitTest.Item.Index);
                    }
                    else if (hitTest.Item.ImageKey.Equals("expandMinus.png"))
                    {
                        collapseItem(hitTest.Item.Index);
                    }
                }
            }
            catch { }
